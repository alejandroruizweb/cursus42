...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/13 13:48:37 by alruiz-c          #+#    #+#             */
/*   Updated: 2022/12/13 13:58:41 by alruiz-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || fd > 4096)
		return (0);
	memo[fd] = ft_memo_read(fd, memo[fd]);
	if (!memo[fd])
		return (0);
	return_line = ft_return_line(memo[fd]);
	memo[fd] = ft_update_memo(memo[fd]);
	return (return_line);
}
